...
function cargarJson() {
    $.ajax({
        url: "http://qando.es/docs/pokemons.php", // Llamo al archivo JSON con los datos 
        type: "get",
        dataType: "json",
        cache: false,
        contentType: false,
        ...
        .done(function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                datos += "<div class='col-xl-2'> <img src=" +  data[i].image + "><label>"+data[i].race+"</label><label>"+data[i].id+"</label><label>"+data[i].pokemonNumber+"</label></div><label>"+data[i].id+"</label>";
            }
            datos += "</div></div";
