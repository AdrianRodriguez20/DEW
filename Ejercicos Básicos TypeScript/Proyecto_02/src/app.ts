...
*/
function mifuncion(x:number):string {
...
*/
for (let i:number = 0, j:number = 20; (j > 0) && (i < 9); i++, j = j - 3) {
    console.log(i, j);
};
...
*/
function letraDNI(numDNI:number):string { //el usuario introduce el DNI
    let letraDNI:string = "";
    let resto:number;
    let cadena:string = "TRWAGMYFPDXBNJZSQVHLCKE";
...
*/
function conversorTemp(gradosFarenheit:number):number {
    let resultadoCelsius:number;
...
*/
function conversorTemperatura(gradosFarenheit:number, simbolo:boolean):string {
    let resultadoCelsius:number;
    var simbolo:boolean;
    var solucion:string;
    resultadoCelsius = (gradosFarenheit - 32) * (5 / 9);
...
*/
function desgloseBilletes(cantidadDinero:number):void {


    var cash:number[] = Array(500, 200, 100, 50, 20, 10, 5, 2, 1);  // En este array están todos los billetes y monedas.

    var cambio:number[] = Array(0, 0, 0, 0, 0, 0, 0, 0, 0);  // Y en este otro los billetes y monedas a devolver


    for (var i:number = 0; i < cash.length; i++) { // Recorremos los billetes y monedas
...
var dia:number = momentoActual.getDay();
var mes:number= momentoActual.getMonth();
var anio:number = momentoActual.getFullYear();

var hora :number= momentoActual.getHours();
var minuto:number = momentoActual.getMinutes();
var segundo :number= momentoActual.getSeconds();


var horaImprimible:string = hora + " : " + minuto + " : " + segundo;
var fechaImprimible:string = dia + " / " + mes + " / " + anio;
...
var fecha: Date = new Date("02/04/2015");
console.log(fecha);
var fechaDos: Date = new Date("2015-04-02");
console.log(fechaDos);
var fechaTres: Date = new Date("April 02, 2015");
...
function tiempoTranscurrido(fechaNacimiento:string, unidad:string) {
    var resultado;
    var fechaUsuario:Date = new Date(fechaNacimiento);
    var fechaActual:Date = new Date();
    var diferenciaMs:number = Number(fechaActual) - Number(fechaUsuario); // new Date nos da el tiempo en milisegundos , así que restamos la fecha actual entre la que nos da el usuario
...
*/
function hdec2hms(x:string):string {

    var arrayHora:string[] = x.split("."); //con el split separamos la hora que nos da el usuario  y lo guardamos en un array
    var hora:number = Number(arrayHora[0]);  // la posición 0 , es la hora 
    var minuto:number = (Number(arrayHora[1]) * 0.6); // a continuación hacemos la conversión a minutos 
    var segundo:number = ((Number(arrayHora[1]) * 0.6) - (minuto)) * 100; // y después a segundos 
...
function inversaHdec2hms(x:string):string {
    var arrayHora:string[] = x.split(":");  //con el split separamos la hora , minutos y segundos.
    var hora:number = Number(arrayHora[0]); // la posición 0 , es la hora 
    var decimal:number = (Number(arrayHora[1]) + (Number(arrayHora[2]) / 60) / 60); //pasamos los minutos y segundos a horas.
    var decimalImprimir:RegExpMatchArray|null = String(decimal).match(/.[0-9][0-9]/); // y mostramos solo dos caracteres
    var horaImprimible:string = hora +""+ decimalImprimir;
...
function fechaJuliana(fechaGregoriana:string):number {
    var fechaUsuario:Date = new Date(fechaGregoriana); //guardamos la fecha que  nos da el usuario 
    var fechaUnix:Date = new Date('1970-1-1'); // y ka correspondiente a la fecha UNIX
    var diasMs:number = 86400000; 
    var diferenciaMs:number = Number(fechaUsuario) -  Number(fechaUnix); //restamos ambas fechas 
    var diferenciaEnDias:number= diferenciaMs / diasMs; // y dividimos los milisegundos en la equivalencia a un día
    var fechaJuliana:number;
...
*/
function sinDegAlt(x:number):number {
...
function cosDegAlt(x:number):number {
